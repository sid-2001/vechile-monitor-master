...
export const vehicleMonitorService = {
  ...
  getRoles: () => api1.get('/roles'),
  createRole: (payload: Record<string, unknown>) => api1.post('/roles', payload),
  updateRole: (id: string, payload: Record<string, unknown>) => api1.put(`/roles/${id}`, payload),
  deleteRole: (id: string) => api1.del(`/roles/${id}`),
